    ...
    class heap
    {
        ...
        void heapifyDown(int index) {
            while(true)
            {
               ...
               if(leftChild < len && nums[leftChild] > nums[index])
                   maxIndex = leftChild;
               if(righttChild< len && nums[righttChild] > nums[index])
        ...
        void pop() {
            ...
            nums[1] = nums.back();
            nums.pop_back();
            if (heap.size() > 1) { // Ensure heap is not empty
                heapifyDown(1);
            }
        }
        bool empty() const {
        ...
        void buildHeap(vector<int>& arr, int n) {//O(n) complexity and not n.logn even though 
            //the  loop takes n time and heapify takes logn time!!!!
            // Step 1: Build a max heap
            for (int i = n / 2; i >= 0; --i) {//for 0 based indexing take i = n/2 - 1 to 0
                heapify(arr, n, i);
            }
        }
